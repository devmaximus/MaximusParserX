...
{
	public class points_of_interest : DumpObjectBase
    {
		...
		{
			return string.Format("INSERT IGNORE INTO `" + TableName + "` (`entry`, `x`, `y`, `icon`, `flags`, `data`, `icon_name`) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}', '{5}', '{6}');", entry.GetValueOrDefault(), ((Decimal)x.GetValueOrDefault()), ((Decimal)y.GetValueOrDefault()), icon.GetValueOrDefault(), flags.GetValueOrDefault(), data.GetValueOrDefault(), icon_name.ToSQL());
		}
