...
{
	public class creature_template_addon : DumpObjectBase
    {
		...
		{
			return string.Format("INSERT IGNORE INTO `" + TableName + "` (`entry`, `mount`, `bytes1`, `b2_0_sheath`, `b2_1_pvp_state`, `emote`, `moveflags`, `auras`) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}', '{5}', '{6}', '{7}');", entry.GetValueOrDefault(), mount.GetValueOrDefault(), bytes1.GetValueOrDefault(), b2_0_sheath.GetValueOrDefault(), b2_1_pvp_state.GetValueOrDefault(), emote.GetValueOrDefault(), moveflags.GetValueOrDefault(), auras.ToSQL());
		}
